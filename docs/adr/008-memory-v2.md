...
### 2. Memory Architecture: Thread vs Long-Term
...
| -------------------- | --------------------------------- | ------------------------------------------------------------------------ |
| **Thread memory**   | OpenAI Agents SDK `SQLiteThread` | In-conversation context, per-turn history                                |
...
Event arrives (user_message | agent_response)
  ...
  │       WHERE type = 'episodic' AND thread_id = ?
...
### 10. Write-Path Agent (Post-Thread / Nightly)
...
Triggered by cron schedule (e.g., `0 3 * * *`). Runs consolidation for any missed threads, then performs maintenance:

```
Nightly maintenance (scheduled)
  │
  ├─ 1. Consolidate any pending threads (same as 10.1)
...
At 50 threads/day: **~$0.05/day**. With a local model (32K context): **free**.
...
```python
{
    ...
    "unconsolidated_threads": 2, # threads awaiting write-path agent
...
### 15. Thread Lifecycle and Consolidation Trigger
...
| `thread.completed` | `context.subscribe_event("thread.completed", handler)` | Trigger write-path agent for post-session consolidation |
...
**Tertiary: Nightly sweep.** The nightly maintenance task (§10.2) consolidates any threads that were missed by both mechanisms above (e.g., process crash after the last message, no subsequent message to trigger detection).
...
schedules:
  - name: nightly_maintenance
    ...
    description: "Consolidate pending threads, apply decay, enrich entities"
...
- **ADR 004 (Event Bus)** — This ADR introduces `thread.completed` as a new kernel-published EventBus topic (core change C1, §15). `MessageRouter` publishes this event when a session rotates due to inactivity. Memory v2 subscribes to it as the primary consolidation trigger. Other extensions can subscribe to the same topic for their own session-boundary logic.
