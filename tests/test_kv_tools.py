...
@asynccontextmanager
async def _kv_extension_context(
    tmp_path: Path,
    settings: AppSettings | None = None,
) -> AsyncIterator[tuple[object, Path]]:
    """Load sandbox extensions and yield initialized kv extension with data dir."""
...
async def test_kv_set_returns_structured_result(tmp_path: Path) -> None:
    ...
    async with _kv_extension_context(tmp_path) as (kv_ext, _data_dir):
        tools = kv_ext.get_tools()
        kv_set = next(t for t in tools if getattr(t, "name", None) == "kv_set")
        args = json.dumps(
            {"key": "test_key", "value": "test_value"}, ensure_ascii=False
        )
...
async def test_kv_concurrent_sets_persist_all_keys(tmp_path: Path) -> None:
    ...
    async with _kv_extension_context(tmp_path) as (kv_ext, data_dir):
        tools = kv_ext.get_tools()
        kv_set = next(t for t in tools if getattr(t, "name", None) == "kv_set")
        n = 30
        ...
        async def _set(i: int) -> None:
            ...
            result = await kv_set.on_invoke_tool(
                _make_tool_ctx(kv_set.name, args), args
            )
...
async def test_kv_max_entries_blocks_new_key(tmp_path: Path) -> None:
    ...
    async with _kv_extension_context(tmp_path, settings=settings) as (
        kv_ext,
        _data_dir,
    ):
        tools = kv_ext.get_tools()
...
async def test_kv_corrupt_file_logs_and_reads_empty(tmp_path: Path, caplog) -> None:
    ...
    async with _kv_extension_context(tmp_path) as (kv_ext, data_dir):
        ...
        with caplog.at_level(logging.WARNING):
            ...
            result = await kv_get.on_invoke_tool(
                _make_tool_ctx(kv_get.name, args), args
            )
